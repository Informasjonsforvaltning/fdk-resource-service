...
class UnionGraphControllerTest : BaseControllerTest() {
    ...
    fun `updateOrder should return 404 when order not found`() {
        ...
        every {
            unionGraphService.updateOrder(id = orderId, fields = any())
        } returns null
    ...
    fun `updateOrder should return 400 when updateTtlHours is invalid`() {
        ...
        every { unionGraphService.updateOrder(any(), any<UnionGraphService.UpdateFields>()) } throws
            IllegalArgumentException("updateTtlHours must be 0 (never update) or at least 24")
    ...
    @Test
    fun `getAvailableOrders should return list of available union graphs`() {
        // Given
        val orders =
            listOf(
                UnionGraphOrder(
        ...
        // When & Then
        mockMvc
            .perform(get("/v1/union-graphs/available"))
            .andExpect(status().isOk)
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$[0].id").value("order-1"))
            .andExpect(jsonPath("$[0].name").value("Order 1"))
            .andExpect(jsonPath("$[0].resourceTypes[0]").value("CONCEPT"))
            .andExpect(jsonPath("$[1].id").value("order-2"))
            .andExpect(jsonPath("$[1].name").value("Order 2"))
            .andExpect(jsonPath("$[1].resourceTypes[0]").value("DATASET"))
    }

    @Test
    fun `getAvailableOrders should return empty list when no graphs available`() {
        // Given
        every { unionGraphService.getAvailableOrders() } returns emptyList()
    ...
    @Test
    fun `getOrderInfo should return minimal info for available graph`() {
        // Given
        val order =
            UnionGraphOrder(
                id = "test-order-info",
        ...
        // When & Then
        mockMvc
            .perform(get("/v1/union-graphs/test-order-info/info"))
            .andExpect(status().isOk)
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.id").value("test-order-info"))
            .andExpect(jsonPath("$.name").value("Test Order Info"))
            .andExpect(jsonPath("$.description").value("Test description"))
            .andExpect(jsonPath("$.resourceTypes[0]").value("CONCEPT"))
            .andExpect(jsonPath("$.resourceTypes[1]").value("DATASET"))
    }

    @Test
    fun `getOrderInfo should return 404 when order not found`() {
        // Given
        every { unionGraphService.getOrder("non-existent") } returns null

        // When & Then
        mockMvc
            .perform(get("/v1/union-graphs/non-existent/info"))
            .andExpect(status().isNotFound)
    }

    @Test
    fun `getOrderInfo should return 404 when order not available`() {
        // Given
        val order =
            UnionGraphOrder(
                id = "test-order-no-data",
                name = "Test Order No Data",
                status = UnionGraphOrder.GraphStatus.PENDING,
            )

        every { unionGraphService.getOrder("test-order-no-data") } returns order

        // When & Then
        mockMvc
            .perform(get("/v1/union-graphs/test-order-no-data/info"))
            .andExpect(status().isNotFound)
    }
}
