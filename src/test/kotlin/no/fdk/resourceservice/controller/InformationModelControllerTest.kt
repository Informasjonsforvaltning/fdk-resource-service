...
class InformationModelControllerTest : BaseControllerTest() {
    ...
    @Test
    fun `findInformationModels should return 400 when ids list exceeds 100 items`() {
    ...
    @Test
    fun `findInformationModels should return 200 when ids list has exactly 100 items`() {
        val ids = (1..100).map { "id-$it" }
        val resources = ids.map { mapOf("id" to it, "title" to "Information Model $it") }
        val idsJson = ids.joinToString(",") { "\"$it\"" }
        val requestBody = """{"ids": [$idsJson]}"""

        every { resourceService.getResourceJsonListById(ids, ResourceType.INFORMATION_MODEL) } returns resources
