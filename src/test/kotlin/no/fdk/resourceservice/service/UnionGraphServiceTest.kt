...
class UnionGraphServiceTest {
    ...
    fun `createOrder should return existing order when found`() {
        ...
        // When
        val result = unionGraphService.createOrder(resourceTypes, 24, "https://example.com/webhook", null, false, name = "Test Order")
    ...
    fun `createOrder should throw IllegalArgumentException when updateTtlHours is 1`() {
        ...
        assertTrue(exception.message!!.contains("updateTtlHours must be 0 (never update) or at least 24"))
    }
    ...
    fun `createOrder should accept updateTtlHours of 24`() {
        ...
        // Then
        assertNotNull(result)
        assertEquals(24, result.order.updateTtlHours)
    }
    ...
    fun `updateOrder should throw IllegalArgumentException for invalid updateTtlHours`() {
        ...
        val exception =
            assertThrows<IllegalArgumentException> {
                unionGraphService.updateOrder(id = orderId, updateTtlHours = 23)
            }
        assertTrue(exception.message!!.contains("updateTtlHours must be 0 (never update) or at least 24"))
    }
    ...
    @Test
    fun `updateOrder should detect resourceTypes change and reset to PENDING`() {
        // Given
        val orderId = "test-order-id"
        val existingOrder =
            UnionGraphOrder(
    ...
    fun `buildUnionGraph should return false when no resources found`() {
        ...
        // When
        val result = unionGraphService.buildUnionGraph(listOf(ResourceType.CONCEPT), orderId = "test-order-id")
    ...
    fun `processOrder should mark order as failed when lock fails`() {
        ...
        // Then
        verify(exactly = 0) { unionGraphOrderRepository.markAsCompleted(any()) }
    ...
    fun `createOrder should persist resourceIds`() {
        ...
        // Then
        assertTrue(result.isNew)
        assertEquals(resourceIds.sorted(), result.order.resourceIds)
        verify { unionGraphOrderRepository.save(any()) }
    }
    ...
    fun `createOrder should persist resourceUris`() {
        ...
        // Then
        assertTrue(result.isNew)
        assertEquals(resourceUris.sorted(), result.order.resourceUris)
        verify { unionGraphOrderRepository.save(any()) }
    }
    ...
    fun `createOrder should treat different resourceIds as different orders`() {
        ...
        assertFalse(result1.isNew)
        assertFalse(result2.isNew)
        assertEquals(order1.id, result1.order.id)
        assertEquals(order2.id, result2.order.id)
    }
