...
class UnionGraphProcessorTest {
    ...
    fun `processPendingOrders should process orders when found`() {
        // Given
        val lockTimeout = java.sql.Timestamp.from(Instant.now().minus(60, ChronoUnit.MINUTES))
        val orders =
            listOf(
                UnionGraphOrder(
                    id = "order-1",
    ...
    fun `processExpiredOrders should reset expired orders to pending`() {
        ...
        every {
            unionGraphOrderRepository.findByStatus(UnionGraphOrder.GraphStatus.COMPLETED.name)
    ...
    fun `cleanupStaleLocks should release stale locks`() {
        ...
        val staleOrder =
            UnionGraphOrder(
                ...
                lockedAt = Instant.now().minus(49, ChronoUnit.HOURS),
            )
        val freshOrder =
            UnionGraphOrder(
                id = "fresh-order",
        ...
        every {
            unionGraphOrderRepository.findByStatus(UnionGraphOrder.GraphStatus.PROCESSING.name)
