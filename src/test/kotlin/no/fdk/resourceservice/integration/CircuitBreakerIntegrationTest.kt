...
class CircuitBreakerIntegrationTest : BaseIntegrationTest() {
    ...
    fun `circuit breaker should open and pause listener when failure threshold is reached`() {
        ...
        assertEquals(CircuitBreaker.State.CLOSED, conceptCircuitBreaker.state, 
        ...
        try {
            ...
            val triggerEvent = ConceptEvent.newBuilder()
            ...
            if (actualFailureRate >= 0.6f) {
                ...
                if (finalState == CircuitBreaker.State.OPEN) {
                    ...
                    status!!.listeners.forEach { (listenerId, listenerStatus) ->
                        ...
                        assertFalse(listenerStatus.isRunning, 
                            "Listener $listenerId should be paused when circuit breaker is OPEN")
                        assertTrue(listenerStatus.isPaused, 
    ...
    fun `circuit breaker should resume listener when it closes`() {
        ...
        val pauseMethod = KafkaListenerManager::class.java.getDeclaredMethod(
        ...
        val resumeMethod = KafkaListenerManager::class.java.getDeclaredMethod(
    ...
    private fun getConceptListeners(): List<KafkaMessageListenerContainer<*, *>> {
        return listOf(mockListenerContainer)
    }
}
