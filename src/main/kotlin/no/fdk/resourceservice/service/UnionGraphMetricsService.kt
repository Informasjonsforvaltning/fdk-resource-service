...
class UnionGraphMetricsService(
    ...
    private val dataServicesExpandedCounter: Counter =
        Counter.builder("union_graph_data_services_expanded_total")
    ...
    private val processingDurationTimer: Timer =
        Timer.builder("union_graph_processing_duration_seconds")
            .description("Duration of union graph processing in seconds")
            .register(meterRegistry)

    private val webhookCallTimer: Timer =
        Timer.builder("union_graph_webhook_call_duration_seconds")
    ...
    private val pendingOrdersGauge: Gauge =
        Gauge.builder("union_graph_orders_pending", java.util.function.Supplier {
    ...
    private val processingOrdersGauge: Gauge =
        Gauge.builder("union_graph_orders_processing", java.util.function.Supplier {
    ...
    private val completedOrdersGauge: Gauge =
        Gauge.builder("union_graph_orders_completed", java.util.function.Supplier {
    ...
    private val failedOrdersGauge: Gauge =
        Gauge.builder("union_graph_orders_failed", java.util.function.Supplier {
     ...
     */
    fun recordWebhookCall(durationSeconds: Double, success: Boolean) {
        webhookCallTimer.record((durationSeconds * 1000).toLong(), java.util.concurrent.TimeUnit.MILLISECONDS)
        Counter.builder("union_graph_webhook_calls_total")
    ...
    private fun getOrderCountByStatus(status: UnionGraphOrder.GraphStatus): Double {
        return orderRepository?.countByStatus(status)?.toDouble() ?: 0.0
    }
    ...
    private fun updateProgressGauges(orderId: String) {
        ...
        try {
            meterRegistry.remove(
                meterRegistry.find("union_graph_processing_progress_ratio")
                    .tag("order_id", orderId)
                    .gauge(),
            )
            meterRegistry.remove(
                meterRegistry.find("union_graph_processing_resources_processed")
                    .tag("order_id", orderId)
                    .gauge(),
            )
            meterRegistry.remove(
                meterRegistry.find("union_graph_processing_resources_total")
                    .tag("order_id", orderId)
                    .gauge(),
            )
        } catch (e: Exception) {
        ...
        Gauge.builder("union_graph_processing_progress_ratio", java.util.function.Supplier {
        ...
        Gauge.builder("union_graph_processing_resources_processed", java.util.function.Supplier {
        ...
        Gauge.builder("union_graph_processing_resources_total", java.util.function.Supplier {
