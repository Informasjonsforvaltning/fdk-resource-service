...
class UnionGraphProcessor(
    ...
    fun processPendingOrders() {
        try {
            ...
            // Lock timeout: release locks older than 48 hours (in case of crashed instances)
            val lockTimeout = Instant.now().minus(48, ChronoUnit.HOURS)
            ...
            val orders =
                unionGraphOrderRepository.findNextPendingOrders(
                    java.sql.Timestamp.from(lockTimeout),
                    maxOrdersPerCycle,
                )
    ...
    /**
     * Scheduled task that processes union graph orders incrementally, one batch at a time.
     * Runs every 10 seconds to process batches incrementally, reducing memory consumption.
     *
     * This processes the oldest PROCESSING order (by created_at) that has a processing_state,
     * allowing resumable batch processing that prevents OutOfMemoryError.
     * Only one batch is processed per scheduler run to minimize memory usage.
     */
    @Scheduled(fixedDelay = 10000) // 10 seconds
    fun processIncrementalBatches() {
        try {
            ...
            if (processingOrders.isEmpty()) {
                logger.debug("No PROCESSING orders found for incremental processing")
                return
            }
            ...
            try {
                ...
                if (!shouldContinue) {
                    ...
                    if (updatedOrder != null && updatedOrder.status == UnionGraphOrder.GraphStatus.COMPLETED) {
                        // Order completed successfully
                        logger.info("Order {} completed via incremental processing", oldestOrder.id)
                    } else if (updatedOrder != null && updatedOrder.status == UnionGraphOrder.GraphStatus.FAILED) {
                        logger.warn("Order {} failed during incremental processing", oldestOrder.id)
                    }
                }
            } catch (e: Exception) {
                logger.error("Error processing incremental batch for order {}", oldestOrder.id, e)
            }
        } catch (e: Exception) {
            logger.error("Error in processIncrementalBatches scheduler", e)
        }
    }
    ...
    fun processExpiredOrders() {
        try {
            ...
            val completedOrders =
                unionGraphOrderRepository
                    .findByStatus(UnionGraphOrder.GraphStatus.COMPLETED.name)
     ...
     *
     * Only resets orders to PENDING if they've been locked for more than 48 hours
    ...
    fun cleanupStaleLocks() {
        try {
            // Use 48 hours as the lock timeout (matching processPendingOrders)
            val lockTimeout = Instant.now().minus(48, ChronoUnit.HOURS)
            logger.debug("Cleaning up stale locks older than {} (48 hours)", lockTimeout)

            // Find orders that are locked but haven't been updated recently
            // Only reset to PENDING if locked for more than 48 hours
            val staleOrders =
                unionGraphOrderRepository
                    .findByStatus(UnionGraphOrder.GraphStatus.PROCESSING.name)
            ...
            for (order in staleOrders) {
                logger.warn(
                    "Releasing stale lock on order {} (locked by {} at {}, locked for more than 48 hours)",
                    order.id,
                    order.lockedBy,
                    order.lockedAt,
                )
            ...
            if (staleOrders.isNotEmpty()) {
                logger.info("Released {} stale locks (locked for more than 48 hours)", staleOrders.size)
            }
        } catch (e: Exception) {
